...
class ImageAttacker():
    ...
    def __init__(self, epsilon, norm_type=NormType.Linf, random_init=True, cls=True, *args, **kwargs):
        ...
        if self.bounding is None:
            self.bounding = (0, 1)
        self.epsilon_per_iter = kwargs.get('step_size')
    ...
    def attack(self, image, num_iters):
        ...
        epsilon_per_iter = self.epsilon_per_iter
    ...
    def run_trades(self, net, image, num_iters,ind_prompt=None):
        with torch.no_grad():
            origin_output = net.module.inference_image(self.preprocess(image),ind_prompt)
            if self.cls:
        ...
        for i in range(num_iters):
            ...
            adv_output = net.module.inference_image(image_adv,ind_prompt)
            if self.cls:
